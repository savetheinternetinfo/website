...
class MemeController {
    ...
    private memePath: string;

    constructor() {
        this.cache = new CacheService(600);
        this.memePath = path.resolve(__dirname, "../..", config.memePath);
    }
    ...
    private getImages(): Promise<Image[]> {
        return this.cache.get<Image[]>("images", (): Promise<Image[]> =>
            fs.readdir(this.memePath).then(galleryFiles =>
                Promise.all(galleryFiles.map(f => path.resolve(this.memePath, f))
